...
> **Implementation status:** The `ingredients.calories` column still exists in the DB schema and is read/written by `src/lib/recipes.ts`. The migration removing it is tracked in the `feat/serving-unit-refinement` branch and will land in a follow-up merge.
