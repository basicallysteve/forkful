...
## For You Section
A section at the top of the recipes list page, visible to logged-in Users who have at least one Cuisine Preference set **or** a Nutrition Goal set. Shows up to 5 public, published Recipes ranked by two **co-equal** signals — Cuisine Type match against Cuisine Preferences, and Goal Fit (see below) — with the Dietary Restriction filter always applied as a hard filter on top. A candidate qualifies by matching cuisine **or** fitting the goal, so a goals-only User (no cuisine preferences) still gets goal-driven recommendations and a cuisine-only User is unaffected. Cuisine and Goal Fit start as roughly equal contributors, tuned later. Acts as both a discovery feature for other authors' recipes and a nudge toward the User's daily targets.
_Note_: originally cuisine-only; Goal Fit was added as a co-equal driver when Food Log Entry / Nutrition Goal shipped.

## Goal Fit
A live, **remaining-aware** ranking signal that favors Recipes helping a User hit the day's Nutrition Goal, used by the For You Section only (the Recipe Suggestion Emails are deliberately left on their existing signals — "remaining today" is a live concept that does not translate to a point-in-time digest). Computed against **remaining budget** = Nutrition Goal − the day's logged intake (summed from today's Food Log Entries), so it changes through the day. Scoring is **gap-fill with a calorie ceiling**: a Recipe's Per-Serving Nutrition is rewarded for closing the macros the User is short on — **protein weighted highest** (treated as a floor to reach) — and **penalized when per-serving calories exceed remaining calories** (calories treated as a ceiling, not a gap to fill). Deliberately explainable in one sentence so the section can label *why* a Recipe was surfaced (e.g. "High protein · fits your remaining calories"). Only Recipes that are Nutrition Complete **and** have Serves set are eligible (both required for trustworthy Per-Serving Nutrition); ineligible Recipes get no Goal Fit boost but may still appear via cuisine match. When remaining calories are already ≤ 0 (the User has hit their calorie cap), Goal Fit stops boosting and, if anything, favors the lowest-calorie options — it never cheerleads eating more once capped. Inert when the User has no Nutrition Goal, leaving the For You Section cuisine-driven as before.
_Avoid_: macro match, goal score, nutrition match, diet fit
...
## Pantry-Gap Fill
An "add what I'm missing" action that populates the active Shopping List from a Recipe. Each Recipe Ingredient is matched against current Pantry stock using the same Food-matching logic as Meal Preparation Deduction (a `'food'` pantry item matches on `foodId`; a `'product'` pantry item matches on its Product's `parentFoodId`). For any ingredient whose required quantity is not fully covered by matching stock, a `'food'` Shopping List Item referencing that ingredient's Food is added with a quantity equal to the shortfall (in the ingredient's unit). Only stock that can be converted into the ingredient's unit is credited toward coverage; stock in a unit that can't be converted (e.g. an Uncalibrated Custom Unit or an otherwise unconvertible unit) is treated as covering nothing, so a mix credits the convertible part and an ingredient with no convertible stock falls back to its full required quantity. Ingredients already fully in stock are skipped. This is the only recipe-driven input method in v1; whole-recipe explosion (adding every ingredient regardless of stock) is deferred.
...
## Food Log Entry
A single record of something a User consumed. Primarily a **nutrition ledger entry**, not a pantry operation: it captures what was eaten and its macro contribution, independent of whether the item was ever in stock. It references a Food, Product, or Prepared Meal for its nutrition, and **optionally** links to a Pantry Item — when that link is present, logging also deducts stock; when absent (e.g. a restaurant meal), the entry records nutrition only. This mirrors how a Pantry Item optionally links back to a Shopping List Item (provenance without hard coupling). Two mechanisms can therefore change pantry stock — Meal Preparation Deduction and Food Log Entry — and the boundary between them is defined per those terms. A log entry has a `sourceType` discriminator (`'food' | 'product' | 'prepared_meal'`), consistent with the discriminators on Pantry Item and Shopping List Item. Raw-Recipe logging (logging a Recipe never cooked into a Prepared Meal) and freeform text entries are deliberately out of scope for v1. The five core macros (calories, protein, carbs, fat, fiber) are **snapshotted** onto the entry at log time — frozen facts, not recomputed from the referent's current nutrition — so daily totals are stable, cheap to sum, and survive edits or soft-deletion of the referent Food/Product/Recipe. The referent FK and the resolved amount + unit are retained for provenance and display; correcting a mis-logged entry means editing the entry, not the Food.

A Food Log Entry is expressed as an **amount + unit** (unit constrained to the referent's Measurements; always servings for a Prepared Meal), and its macros are computed by reusing `calculateCalories` / `convertUnit` and applying the same scale factor to all five macros, then frozen. When the entry is pantry-linked, the logged amount **is** the deducted amount — one physical quantity, not two independent numbers. Logging more than the Pantry Item's current stock never blocks (you can eat more than a stale count shows): the entry records the full eaten amount's nutrition and the deduction floors at 0, mirroring Meal Preparation Deduction's "proceed anyway".

If the chosen unit is an Uncalibrated Custom Unit, macros would compute to zero — a silent hole in a calorie tracker. Rather than tolerate that, the log flow offers to **open the referent Food/Product inline and supply the missing calibration** (a gram-weight for that custom unit, or serving size/unit + per-serving macros — by weight, volume, or "serving"), reusing the inline-nutrition-entry instinct of the Barcode Creation Modal; the correction persists on the canonical Food/Product and real macros are then frozen onto the entry. If the User declines to calibrate, the entry is logged anyway with zero macros and flagged nutrition-incomplete (a loud signal on the day's total), never silently — logging is never a dead end, but the tracker refuses to be quietly wrong.

When the optional Pantry Item link is present, creating the entry deducts stock **fire-and-forget**, exactly like Meal Preparation Deduction: the deduction happens once at create time (`GREATEST(0, current - amount)`) and is never reversed. Editing or deleting a Food Log Entry changes the ledger only — it does **not** restore or re-true pantry stock; a mis-logged deduction is corrected by editing the Pantry Item directly. The entry stores the deducted amount + unit for display ("deducted 1 serving from pantry") but makes no promise to undo it. This keeps the app's stock model strictly one-directional (no stock-restoring path exists anywhere).

A Food Log Entry is **soft-deleted** (`dateDeleted`), consistent with the content-entity convention (Recipe, Pantry Item, Review) rather than the hard-delete used for Remove Item. Amount, unit, Meal Slot, and Log Date are editable in place; changing the referent is a delete + re-add, not an in-place swap. Editing amount or unit **re-computes and re-freezes** the macros from the referent's *current* nutrition — freezing protects a record from ambient change (someone else editing the Food), but an explicit edit of this entry is a deliberate correction, so recomputing now is correct. If the referent Food/Product/Recipe was since soft-deleted, amount/unit can no longer recompute and become non-editable (Meal Slot, Log Date, and delete still work); the entry survives on its snapshot, the same orphan behaviour as a Prepared Meal outliving its recipe.

**Boundary with Meal Preparation Deduction** (no double-counting): *preparing* a recipe deducts its raw ingredient Pantry Items and produces a Prepared Meal measured in servings; *logging* consumption deducts the thing actually eaten — a Food/Product Pantry Item's stock, or a Prepared Meal's servings — never the raw ingredients a second time. Eating a raw Food (e.g. a banana) deducts that Food's Pantry Item directly.
_Avoid_: consumption, intake record, diary entry, meal log

## Daily Log
A User's Food Log Entries for a single calendar day — a **derived view, not a stored entity** (there is no `daily_logs` table). A day has no lifecycle or status of its own (contrast the Shopping List, which is an entity precisely because it does), so "today's log" is simply the User's entries filtered by `logDate`. Daily totals (calories + the five macros) are summed on the fly from each entry's snapshotted macros; they are never cached in a row.

## Log Date
The user-local calendar day a Food Log Entry is filed under, stored as a plain `date` column (`logDate`). Day boundaries are drawn in the **User's local timezone, never UTC** — bucketing by UTC would misfile evening meals into the next day for users west of UTC. The local day is **derived on the client** (`Intl.DateTimeFormat().resolvedOptions().timeZone` / local `toLocaleDateString`) and sent with each write; no `users.timezone` column and no geolocation permission are used — the browser is the only party that already knows the user's day, for free and with no prompt. Only the date is stored (no time-of-day / `consumedAt` timestamp); the row's inherited `dateAdded` timestamp provides audit and within-day ordering. Defaults to the user's today but is editable so past days can be backfilled.
_Avoid_: consumed date, entry date, log timestamp

## Meal Slot
A user-defined, ordered, named bucket for grouping a day's Food Log Entries (e.g. "Breakfast", "Dinner", "Dessert"). **Not a fixed vocabulary** — the set is per-User and adjustable, so someone eating one meal a day and someone eating five each shape their own slots. The set is stored as an ordered `jsonb` string array on the User, seeded at account creation with `["Breakfast", "Lunch", "Dinner", "Snack"]` and editable in profile/settings — the same shape and pattern as Cuisine Preferences and Dietary Restrictions (no `meal_slots` table). A Food Log Entry stores its slot as a **string snapshot** (`mealSlot`, nullable), not a FK: renaming or deleting a slot never rewrites or orphans past entries, consistent with the snapshot philosophy of Food Log Entry and Prepared Meal. A Daily Log renders its entries grouped by slot in the User's configured order; entries whose label is no longer in the User's current set fall to the end under an "Other" group, and entries with no slot render under "Unsorted". Assigning a slot is always optional — logging never requires one.
_Avoid_: meal, meal type, meal category, meal section
_Distinct from_: **Meal type** (the `meal` property of a Recipe — what kind of dish it is, e.g. a "dinner recipe"). A Meal Slot is a *when-eaten* grouping chosen by the eater; a leftover dinner-recipe eaten in the morning is a Recipe with Meal type "dinner" logged under the "Breakfast" Meal Slot. Same everyday word, two different concepts.

## Nutrition Goal
A User's daily target for intake: a calorie goal plus optional per-macro goals (protein, carbs, fat, fiber). A per-User preference, **manually set** — the User types the numbers; the app does not compute them from biometrics/activity (a TDEE-style **computed goal** is a deliberately deferred, separate feature). Each field is nullable; with no goal set, a Daily Log shows totals only. When a goal is set, the Daily Log renders progress against it (consumed / goal / remaining) using the summed snapshotted macros of the day's Food Log Entries. The goal is a single standing target, not per-day or per-Meal-Slot.
_Avoid_: calorie target, macro target, TDEE, daily budget
